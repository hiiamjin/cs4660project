...
class CaptureRules:
  ...
  def newRemoteGame(self, address, agents, display):
    self.address = address
    game = ClientGame(address, agents, display, self)
...
def readCommand( argv ):
  ...
  if not options.nographics:
    ...
    args['display'] = graphicsDisplay.PacmanGraphics(options.zoom, capture=True)
  else:
    import textDisplay
